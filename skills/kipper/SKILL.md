...
**`--host` takes an IP address or a hostname.** Pass whichever the operator gave you; there is no need to resolve a name first. From 0.11.1 `kip` resolves a hostname to the server's public IPv4 itself for the gateway registration, and keeps the name for SSH so `~/.ssh/config` aliases still apply. A name with no public IPv4 is refused immediately, before any connection, in a message naming `--host`. On 0.11.0 and earlier a hostname reaches the gateway unresolved and the install fails after preflight with `registering subdomain: gateway: ip must be a public address`, so pass an address on those versions.
...
- `kip node list` — list the cluster's nodes. Kipper supports single-server clusters. `kip node add` is hidden and unsupported: workers require manual networking, storage setup, and maintenance. Avoid it for capacity expansion.
...
- **An app's Git token lives in its own namespace,** as a Secret named by `app.spec.git.credentialsSecret` (`<app>-git-credentials-<digest>`, or `<app>-git-credentials` before it was last rotated, key `token`). The console and `kip credentials list` show global credentials; a per-app token is read with `kip credentials get --app <app>` or straight from the Secret. There is no fallback chain — the build uses only that one secret.
