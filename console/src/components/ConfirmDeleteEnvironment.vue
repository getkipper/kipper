...
const hasApps = computed(() => props.environment.apps.length > 0)
const canConfirm = computed(() => confirmText.value === props.environment.name)
</script>
  ...
  >
    <div class="flex items-start gap-4">
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
          This deletes the <span class="font-mono">{{ environment.namespace }}</span> namespace and everything in it from <span class="font-medium">{{ displayName || projectName }}</span>.
        </p>
      </div>
    </div>
    ...
    <NoticeCallout v-if="hasApps" tone="danger" class="mt-5 p-4">
      <p class="text-sm font-medium text-red-800 dark:text-rose-300">
        {{ environment.apps.length }} running {{ environment.apps.length === 1 ? 'app' : 'apps' }} will be destroyed:
      </p>
      <div class="mt-3 flex flex-wrap gap-1.5">
        <span
          v-for="app in environment.apps"
          :key="app.name"
    ...
    <div class="mt-5">
      <label class="block text-sm text-slate-700 dark:text-slate-300">
        Type <span class="font-mono font-semibold text-slate-900 dark:text-slate-50">{{ environment.name }}</span> to confirm
      </label>
      <input
        v-model="confirmText"
        type="text"
