...
func TestGetClusterByHostMatchesEitherSpelling(t *testing.T) {
	cfg := &Config{Clusters: []Cluster{
		{Name: "lab.kipper.run", Host: "203.0.113.10", GatewayToken: "tok"},
		{Name: "other", Host: "198.51.100.4"},
	}}

	if got := cfg.GetClusterByHost("203.0.113.10", "box.kipper.sh"); got == nil || got.Name != "lab.kipper.run" {
		t.Errorf("the recorded spelling must match, got %v", got)
	}
	if got := cfg.GetClusterByHost("box.kipper.sh", "203.0.113.10"); got == nil || got.Name != "lab.kipper.run" {
