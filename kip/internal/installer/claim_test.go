...
func TestGatewayAddressForAcceptsAPublicIP(t *testing.T) {
	got, err := gatewayAddressFor("203.0.113.10", nil)
...
func TestGatewayAddressForResolvesAHostname(t *testing.T) {
	lookup := func(host string) ([]string, error) {
		...
		return []string{"2001:db8::1", "203.0.113.10"}, nil
	}
	...
	if label := hostnames.LabelForIP(got); label != "203-0-113-10" {
		t.Errorf("label = %q, want 203-0-113-10", label)
	}
}
...
func TestGatewayAddressForRefusesWhatTheGatewayWould(t *testing.T) {
	cases := map[string]struct {
		...
		"a hostname with only IPv6": {
			"v6.example.com",
			func(string) ([]string, error) { return []string{"2001:db8::1"}, nil },
		},
	}
...
func TestClaimRecordsTheTokenUnderTheHostTheOperatorGave(t *testing.T) {
	...
	claimed, err := claimGatewayName(gw, store, "lab", "203.0.113.10", "box.kipper.sh")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestClaimReadsTheTokenUnderTheSameKey(t *testing.T) {
	...
	if _, err := claimGatewayName(gw, store, "lab", "203.0.113.10", "box.kipper.sh"); err != nil {
...
func TestAddressGuardWaitsForAResolvedHost(t *testing.T) {
	if _, _, err := KipperRunLabelFor("203-0-113-10.kipper.run", "box.kipper.sh"); err != nil {
		t.Errorf("an unresolved --host must not trigger the address guard: %v", err)
	}
	// With a real address it still bites.
	if _, _, err := KipperRunLabelFor("203-0-113-10.kipper.run", "198.51.100.9"); err == nil {
		t.Error("another server's derived name must still be refused once the address is known")
	}
	if _, _, err := KipperRunLabelFor("203-0-113-10.kipper.run", "203.0.113.10"); err != nil {
...
func TestGatewayAddressForRefusesLiteralIPv6(t *testing.T) {
	got, err := gatewayAddressFor("2001:db8::1", nil)
	if err == nil {
