...
func testGrant(t *testing.T, now time.Time) Grant {
	t.Helper()
	g, err := NewGrant("uid-1234", "mailhog", "acme-test", "mailhog-acme-test.storefront.com", "PO review", "admin@storefront.com", 72*time.Hour, now)
	if err != nil {
...
func TestGrantCapAndPrune(t *testing.T) {
	...
	list, err := s.List(ctx, "acme-test", "mailhog")
	if err != nil || len(list) != 1 {
...
func TestRevokeAllForServiceFailsClosedOnNonConvergence(t *testing.T) {
	...
	err := s.RevokeAllForService(ctx, "acme-test", "mailhog")
	if err == nil {
