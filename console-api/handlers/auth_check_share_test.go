...
const (
	shareHost       = "mailhog-acme-test.example.com"
...
func shareHandler(t *testing.T) *shareFixture {
	...
	f := &shareFixture{
		...
		uids:  map[string]string{"acme-test/mailhog": shareServiceUID},
	}
...
func (f *shareFixture) mint(t *testing.T) (string, share.Grant) {
	t.Helper()
	g, err := share.NewGrant(shareServiceUID, "mailhog", "acme-test", shareHost, "PO review", "admin@example.com", 72*time.Hour, f.now)
	if err != nil {
...
func TestShare_RecreatedServiceRejected(t *testing.T) {
	...
	f.uids["acme-test/mailhog"] = "uid-recreated-2"
...
func TestShare_DeletedServiceRejected(t *testing.T) {
	...
	delete(f.uids, "acme-test/mailhog")
...
func TestShare_ExpiredParamFallsThrough(t *testing.T) {
	...
	g, err := share.NewGrant(shareServiceUID, "mailhog", "acme-test", shareHost, "", "admin@example.com", time.Hour, past)
	if err != nil {
		t.Fatalf("NewGrant: %v", err)
	}
