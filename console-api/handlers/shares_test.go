...
const sharesTestHost = "mailhog-acme-test.example.com"
...
func sharesFixture(t *testing.T) *Shares {
	...
	svc := &kipperv1.Service{
		ObjectMeta: metav1.ObjectMeta{Name: "mailhog", Namespace: "acme-test", UID: "uid-mailhog-1"},
		Spec:       kipperv1.ServiceSpec{Type: "mailhog"},
	}
	ing := &networkingv1.Ingress{
		ObjectMeta: metav1.ObjectMeta{Name: "mailhog-ui", Namespace: "acme-test"},
...
func TestSharesCreateListRevoke(t *testing.T) {
	...
	router.ServeHTTP(w, sharesRequest(t, http.MethodPost,
		"/services/mailhog/shares?namespace=acme-test",
	...
	w = httptest.NewRecorder()
	router.ServeHTTP(w, sharesRequest(t, http.MethodGet, "/services/mailhog/shares?namespace=acme-test", ""))
	...
	w = httptest.NewRecorder()
	router.ServeHTTP(w, sharesRequest(t, http.MethodDelete, "/services/mailhog/shares/"+created.ID+"?namespace=acme-test", ""))
...
func TestSharesCreateRefusesNonBrowseable(t *testing.T) {
	s := sharesFixture(t)
	db := &kipperv1.Service{
		ObjectMeta: metav1.ObjectMeta{Name: "mydb", Namespace: "acme-test", UID: "uid-db"},
	...
	w := httptest.NewRecorder()
	routeShares(s).ServeHTTP(w, sharesRequest(t, http.MethodPost, "/services/mydb/shares?namespace=acme-test", `{}`))
...
func TestSharesCreateRequiresReconciledIngress(t *testing.T) {
	...
	w := httptest.NewRecorder()
	routeShares(s).ServeHTTP(w, sharesRequest(t, http.MethodPost, "/services/mailhog/shares?namespace=acme-test", `{}`))
...
func TestSharesCreateCapsLifetime(t *testing.T) {
	...
	routeShares(s).ServeHTTP(w, sharesRequest(t, http.MethodPost,
		"/services/mailhog/shares?namespace=acme-test", `{"expires_in":"2000h"}`))
	if w.Code != http.StatusBadRequest {
...
func TestSharesRevokeReportsStorageFailure(t *testing.T) {
	...
	w := httptest.NewRecorder()
	router.ServeHTTP(w, sharesRequest(t, http.MethodPost, "/services/mailhog/shares?namespace=acme-test", `{}`))
	if w.Code != http.StatusCreated {
