...
func TestServiceDeletionRevokesShareLinks(t *testing.T) {
	...
	g, err := share.NewGrant("uid-mailhog-1", "mailhog", "acme-test", "mailhog-acme-test.example.com", "", "admin@example.com", time.Hour, time.Now())
	...
	_, err = r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "mailhog", Namespace: "acme-test"},
	})
	require.NoError(t, err)
	...
	err = r.Get(context.Background(), types.NamespacedName{Name: "mailhog", Namespace: "acme-test"}, &gone)
...
func TestServiceDeletionFailsClosedWithoutGrantStore(t *testing.T) {
	...
	require.NoError(t, r.Get(context.Background(), types.NamespacedName{Name: "mailhog", Namespace: "acme-test"}, &still))
