...
// writeDexConfig applies a non-empty render and reports whether it was written.
// It checks the live config still matches the render's source and allows issuer
// changes only during cutover. The caller starts the cutover deadline after
// this durable write, before the separate Dex restart.
func (r *ClusterIdentityReconciler) writeDexConfig(ctx context.Context, configYAML, liveConfig string, allowIssuerFlip bool) (bool, error) {
	...
	// Compare the current config and carry its resourceVersion into the apply
	// to preserve concurrent password resets. Forced field ownership still respects
	// that version precondition; cached reads alone may miss an intervening write.
