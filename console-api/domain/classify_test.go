...
func TestTargetEquivalent(t *testing.T) {
	if got := TargetEquivalent("hrportal-backend-test", "example.com"); got != "hrportal-backend-test.example.com" {
		t.Errorf("custom base = %q, want hrportal-backend-test.example.com", got)
	}
	...
	if got := TargetEquivalent("", "example.com"); got != "" {
