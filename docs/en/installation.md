...
An older console-api replaces a credential's whole entry when it is edited, so editing any one credential through it clears who may build with every one of them. An upgrade reads the allow-lists before it starts and writes back any that get cleared while it is running, naming each one. A credential that has moved to a different server is named instead of written back, because the token is offered to that host and a credential pointing elsewhere is a different one to allow. Where a token changed as well, the projects stay allowed, the same as any rotation through the console; if you replaced the credential rather than rotating it, `kip credentials revoke` takes the grant back.
