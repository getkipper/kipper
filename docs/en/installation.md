...
An older console-api replaces a credential's whole entry when it is edited, so editing any one credential through it clears who may build with every one of them. An upgrade reads the allow-lists before it starts and writes back any that get cleared while it is running, naming every one it puts projects back on. A credential added while the upgrade is running was never shown for a decision, so it is left allowing nobody and named too. A credential whose server or token changed in the meantime is named instead of written back. A grant lets a project use one token against one host, and the name alone cannot tell a rotated token from a credential someone deleted and recreated under the same name, so the upgrade prints the exact `kip credentials allow` command for each project it used to allow and leaves the decision to you. A credential you approved at the prompt is dropped the same way if it changes hands before the grant is written, and named. If you rotate or replace a shared credential, do it before or after an upgrade rather than during one.

That covers a list cleared during an upgrade. Going back to a Kipper older than 0.14 and editing a credential there is a different matter: the grants go, and rolling forward again does not bring them back. Run `kip credentials list` after a rollback and grant what is missing. Upgrade before curating a legacy cluster's allow-lists by hand: a credential you grant or revoke first counts as decided, and the upgrade will not add the other projects that were building with it.
